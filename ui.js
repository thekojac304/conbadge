...
(function(){
  ...
  panel.id = 'anim-tuner';
  panel.style.cssText =
    // bottom leaves room for the docked #kf-bar (max 132px + its 10px offset +
    // gap + safe-area) so the slider panel and the timeline bar never overlap.
    'position:fixed;right:8px;top:8px;width:268px;bottom:calc(158px + env(safe-area-inset-bottom));'+
    'overflow:auto;z-index:9999;background:rgba(12,16,24,.94);color:#e8eefc;border:1px solid #2b3a55;'+
  ...
  let html =
    ...
    // activeGroup in buildBones()) — it holds no state of its own. To rip
    // this out: delete this #tn-regions block + its <style>, the
    // activeGroup variable + its one `if` in buildBones(), the
    // regionHasBones()/updateRegionState() functions, and the wiring block
    // marked "picker wiring" further down. Nothing else changes. Reuses the
    // app's own .chip/.is-on styling (same as the test bar) so it matches the
    // rest of the UI instead of introducing a new visual language.
    '<style>#anim-tuner .chip{padding:6px 10px;font-size:12px;border-radius:9px}'+
      '#anim-tuner .chip:disabled{opacity:.32;pointer-events:none}</style>'+
    '<div id="tn-regions" style="display:flex;flex-wrap:wrap;gap:5px;margin-bottom:6px">'+
      '<button class="chip is-on" data-group="">All</button>'+
      '<button class="chip" data-group="Body / head">Head/Torso</button>'+
      '<button class="chip" data-group="Left arm">L Arm</button>'+
      '<button class="chip" data-group="Right arm">R Arm</button>'+
      '<button class="chip" data-group="Left hand — fingers">L Hand</button>'+
      '<button class="chip" data-group="Right hand — fingers">R Hand</button>'+
      '<button class="chip" data-group="Left leg">L Leg</button>'+
      '<button class="chip" data-group="Right leg">R Leg</button>'+
      '<button class="chip" data-group="Ears">Ears</button>'+
      '<button class="chip" data-group="Tail">Tail</button>'+
    '</div>'+
    // ===== end removable block =====================================
  ...
  // Keyframe/timeline controls live in a SEPARATE bottom-docked bar (#kf-bar),
  // not in the scrolling slider panel — so you can pose (sliders, right panel)
  // and capture/scrub/play (always-visible bar) without scrolling between them.
  // Element IDs are unchanged, so every handler below works regardless of which
  // container holds them. The bar is shown/hidden with the Tuner (and hides the
  // test bar while open to free the bottom edge).
  const kfBar = document.createElement('div');
  kfBar.id = 'kf-bar';
  kfBar.style.cssText =
    'position:fixed;left:8px;right:8px;bottom:max(10px,env(safe-area-inset-bottom));z-index:9998;'+
    'max-height:132px;overflow:auto;background:rgba(12,16,24,.95);color:#e8eefc;'+
    'border:1px solid #2b3a55;border-radius:10px;padding:8px 10px;'+
    'font:12px/1.35 system-ui,sans-serif;box-shadow:0 6px 24px rgba(0,0,0,.5);display:none;';
  kfBar.innerHTML =
    '<style>'+
      '#kf-bar select,#kf-bar input[type=text],#kf-bar input[type=number]{background:#1c2740;'+
        'color:#e8eefc;border:1px solid #2b3a55;border-radius:6px;padding:5px}'+
      '#kf-bar .chipbtn{padding:6px 9px;border-radius:6px;border:1px solid #2b3a55;'+
        'background:#1c2740;color:#e8eefc;cursor:pointer}'+
      ...
      '.kf-mk{position:absolute;bottom:4px;width:14px;height:14px;margin-left:-7px;'+
    ...
    '<div style="display:flex;flex-wrap:wrap;gap:8px;align-items:center">'+
      '<b style="color:#9fb3d9">Clip</b>'+
      '<span>t</span><input type="number" id="kf-time" value="0" step="0.1" style="width:52px">'+
      btn('kf-cap','＋ Capture')+
      '<label style="white-space:nowrap"><input type="checkbox" id="kf-loop"> loop</label>'+
      btn('kf-play','▶ Play')+ btn('kf-stop','■ Stop')+
      '<span style="flex:1;min-width:8px"></span>'+
      '<input type="text" id="kf-name" placeholder="clip name" style="width:108px">'+ btn('kf-save','Save')+
      '<select id="kf-lib" style="max-width:130px"></select>'+ btn('kf-load','Load')+ btn('kf-del','Del')+
    '</div>'+
    ...
    '</div>';
  document.body.appendChild(kfBar);
  const testBarEl = document.getElementById('test-bar');
  ...
  function buildBones(){
    ...
    if (!names.length){ sel.innerHTML = '<option value="">— load an avatar first —</option>'; updateRegionState(); return; }
    ...
    updateRegionState();      // REMOVABLE (picker): sync chip highlight/disabled state
  }
  ...
  function updateRegionState(){
    const wrap = el('tn-regions'); if (!wrap) return;
    wrap.querySelectorAll('[data-group]').forEach(b=>{
      const grp = b.dataset.group || null;    // "" on the "All" chip normalizes to null
      b.classList.toggle('is-on', activeGroup === grp);
      b.disabled = grp !== null && !regionHasBones(grp);
    });
  }
  ...
  // ===== REMOVABLE (picker wiring): body-region chip click handling =====
  el('tn-regions')?.addEventListener('click', ev=>{
    const b = ev.target.closest('[data-group]'); if (!b) return;
    const grp = b.dataset.group || null;   // "" on "All" normalizes to null (clear)
    activeGroup = (activeGroup === grp) ? null : grp;   // click the active one again to clear
